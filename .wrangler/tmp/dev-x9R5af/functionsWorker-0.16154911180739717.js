...
// .wrangler/tmp/pages-6ii52Q/functionsWorker-0.16154911180739717.mjs
...
async function onRequestPost3(context) {
  ...
  try {
    ...
    ).bind(grade, cleanSection, parseInt(set_num), lesson_name, url).run();
...
async function onRequestPut(context) {
  ...
  try {
    ...
    ).bind(grade, cleanSection, parseInt(set_num), lesson_name, url, id).run();
    return new Response(JSON.stringify({ success: true }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
...
async function checkAuth2(request, env, requireAdmin = false) {
  const adminPassword = await env.DB.prepare("SELECT value FROM settings WHERE key = 'admin_password'").first("value");
  if (requireAdmin) {
    const adminToken = request.headers.get("X-Admin-Token");
    return adminToken === adminPassword;
...
async function onRequestGet2(context) {
  const { request, env } = context;
  if (!await checkAuth2(request, env, false)) {
    return new Response(JSON.stringify({ error: "Unauthorized access" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestPost4(context) {
  const { request, env } = context;
  if (!await checkAuth2(request, env, true)) {
    return new Response(JSON.stringify({ error: "Unauthorized admin access" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
    });
  ...
  try {
    ...
    ).bind(type, title, url, icon || null).run();
    return new Response(JSON.stringify({ success: true, id: result.meta.last_row_id }), {
      status: 201,
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestPut2(context) {
  ...
  try {
    ...
    ).bind(type, title, url, icon || null, id).run();
    return new Response(JSON.stringify({ success: true }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
...
async function checkAuth3(request, env, requireAdmin = false) {
  const adminPassword = await env.DB.prepare("SELECT value FROM settings WHERE key = 'admin_password'").first("value");
  if (requireAdmin) {
    const adminToken = request.headers.get("X-Admin-Token");
    return adminToken === adminPassword;
...
async function onRequestGet3(context) {
  const { request, env } = context;
  if (!await checkAuth3(request, env, false)) {
    return new Response(JSON.stringify({ error: "Unauthorized access" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestDelete3(context) {
  const { request, env } = context;
  if (!await checkAuth3(request, env, true)) {
    return new Response(JSON.stringify({ error: "Unauthorized admin access" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestPost6(context) {
  ...
  try {
    ...
    for (const note of notes) {
      await env.DB.prepare("INSERT INTO notes (content, color) VALUES (?, ?)").bind(note.content, note.color || "#fef08a").run();
    }
    await env.DB.prepare("DELETE FROM tasks").run();
    for (const task of tasks) {
      await env.DB.prepare("INSERT INTO tasks (text, completed) VALUES (?, ?)").bind(task.text, task.completed ? 1 : 0).run();
    }
    await env.DB.prepare("DELETE FROM links").run();
    for (const link of links) {
      await env.DB.prepare("INSERT INTO links (type, title, url, icon) VALUES (?, ?, ?, ?)").bind(link.type, link.title, link.url, link.icon || null).run();
    }
    ...
    for (const config of gradeConfigs) {
      for (const sect of config.sections) {
        const matchingTemplates = buttons.filter(
          (b) => b.grade === config.grade && (b.section === sect || sect === null && !b.section)
        );
        for (let setNum = 1; setNum <= 2; setNum++) {
          ...
          for (let btnIdx = 1; btnIdx <= 7; btnIdx++) {
            ...
            insertPromises.push(
              env.DB.prepare("INSERT INTO buttons (grade, section, set_num, lesson_name, url) VALUES (?, ?, ?, ?, ?)").bind(config.grade, sect, setNum, lessonName, url).run()
            );
          }
        }
      }
...
async function checkAuth4(request, env, requireAdmin = false) {
  const adminPassword = await env.DB.prepare("SELECT value FROM settings WHERE key = 'admin_password'").first("value");
  if (requireAdmin) {
    const adminToken = request.headers.get("X-Admin-Token");
    return adminToken === adminPassword;
...
__name(checkAuth4, "checkAuth4");
__name2(checkAuth4, "checkAuth");
async function onRequestGet4(context) {
  const { request, env } = context;
  if (!await checkAuth4(request, env, false)) {
    return new Response(JSON.stringify({ error: "Unauthorized access" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestPut3(context) {
  ...
  try {
    ...
    if (id === void 0 || completed === void 0) {
      return new Response(JSON.stringify({ error: "Missing fields" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    await env.DB.prepare("UPDATE tasks SET completed = ? WHERE id = ?").bind(completed ? 1 : 0, id).run();
    return new Response(JSON.stringify({ success: true }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
...
async function onRequestDelete4(context) {
  const { request, env } = context;
  if (!await checkAuth4(request, env, true)) {
    return new Response(JSON.stringify({ error: "Unauthorized admin access" }), {
      status: 401,
      headers: { "Content-Type": "application/json" }
    });
...
// .wrangler/tmp/bundle-AfATVo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-AfATVo/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.16154911180739717.js.map
